...
from src.sampling import calculate_UUt_svd, sample_theta
...
# KL term of the ELBO
def KL_term(prior_vec, theta_hat, log_sigma_ker, log_sigma_im, eps_samples, eps_ker_samples, rank_ker=None):
    sigma_ker = jnp.exp(log_sigma_ker)
    sigma_im = jnp.exp(log_sigma_im)
    sigma_ker_2 = jnp.pow(sigma_ker, 2)
    sigma_im_2 = jnp.pow(sigma_im, 2)
...
def KL_term_alpha(theta_hat, log_sigma_ker, log_sigma_im, eps_samples, eps_ker_samples, rank_ker=None):
    sigma_ker = jnp.exp(log_sigma_ker)
    sigma_ker_2 = jnp.pow(sigma_ker, 2)
    sigma_im = jnp.exp(log_sigma_im)
    sigma_im_2 = jnp.pow(sigma_im, 2)
...
def main():
    ...
    model_new = SineNet(out_dims=1, hidden_dim=16 ,num_layers=2)
    ...
    def loss_fn(params_opt, UUt, x, y, sample_key, prior_vec, rank_ker=None):
    ...
    num_epochs, log_every, plot_epochs = 4500, 75, [5, 25, 50, 100, 250, 500, 1000, 1500, 2000, 2500, 3000, 3500, 4000]
    params_opt_current, opt_state_current, training_key = params_opt, opt_state, sample_key

    @jax.jit
    def train_step(params_opt, opt_state, key):
        UUt, rank_ker = calculate_UUt_svd(model_fn_vec, params_opt["theta"], x_train, y_train)
    ...
    UUt, _ = calculate_UUt_svd(model_fn_vec, params_opt_current["theta"], x_train, y_train)
